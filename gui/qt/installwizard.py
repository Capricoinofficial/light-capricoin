...
from light_capricoin import Wallet, Wallet_2of2, Wallet_2of3
from light_capricoin import bitcoin
from light_capricoin import util
...
import sys
import threading
from light_capricoin.plugins import always_hook
from light_capricoin.mnemonic import prepare_seed

MSG_ENTER_ANYTHING    = _("Please enter a wallet seed, a master public key, a list of capricoin addresses, or a list of private keys")
...
class InstallWizard(QDialog):
    ...
    def __init__(self, config, network, storage):
        ...
        self.setWindowTitle('Light' + '  -  ' + os.path.basename(self.storage.path))
    ...
    def waiting_dialog(self, task, msg= _("Light is generating your addresses, please wait.")):
        def target():
            task()
    ...
    def network_dialog(self):
        ...
        label = QLabel(_("Light communicates with remote servers to get information about your transactions and addresses. The servers all fulfil the same purpose only differing in hardware. In most cases you simply want to let Light pick one at random if you have a preference though feel free to select a server manually.") + "\n\n" \
    ...
    def run(self, action, wallet_type):
        ...
        if action in ['create', 'restore']:
            if wallet_type == 'multisig':
                ...
                hardware_wallets = map(lambda x:(x[1],x[2]), filter(lambda x:x[0]=='hardware', light.wallet.wallet_types))

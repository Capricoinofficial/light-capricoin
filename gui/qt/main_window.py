#!/usr/bin/env python
#
# Light - lightweight Bitcoin client
...
from light_capricoin.i18n import _, set_language
from light_capricoin.util import block_explorer, block_explorer_info, block_explorer_URL
from light_capricoin.util import print_error, print_msg
...
from light_capricoin.bitcoin import MIN_RELAY_TX_FEE, is_valid
from light_capricoin.plugins import run_hook

import icons_rc

from light_capricoin.util import format_satoshis, format_time, NotEnoughFunds, StoreDict
from light_capricoin import Transaction
from light_capricoin import mnemonic
from light_capricoin import util, bitcoin, commands, Wallet
from light_capricoin import SimpleConfig, Wallet, WalletStorage
from light_capricoin import Imported_Wallet
...
from light_capricoin import ELECTRUM_VERSION
import re
...
from light_capricoin.paymentrequest import PR_UNPAID, PR_PAID, PR_EXPIRED
from light_capricoin.paymentrequest import PaymentRequest, InvoiceStore, get_payment_request, make_payment_request
...
class LightWindow(QMainWindow):
    ...
    def __init__(self, config, network, gui_object):
        ...
        self.setWindowIcon(QIcon(":icons/light-capricoin.png"))
        self.init_menubar()
    ...
    def load_wallet(self, wallet):
        import light_capricoin as light
        self.wallet = wallet
        ...
        title = 'Light-Capricoin %s  -  %s' % (self.wallet.light_version, self.wallet.basename())
    ...
    def backup_wallet(self):
        ...
        if new_path != path:
            try:
                ...
                QMessageBox.critical(None,"Unable to create backup", _("Light was unable to copy your wallet file to the specified location.")+"\n" + str(reason))
    ...
    def init_menubar(self):
        ...
        tools_menu.addAction(_("Light preferences") if sys.platform == 'darwin' else _("Preferences"), self.settings_dialog)
        ...
        help_menu.addAction(_("&Official website"), lambda: webbrowser.open("http://www.capricoin.org"))
    ...
    def show_about(self):
        QMessageBox.about(self, "Light-capricoin",
            _("Version")+" %s" % (self.wallet.light_version) + "\n\n" + _("Light Capricoin wallet's focus is speed, with low resource usage and simplifying Capricoin. You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper. Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the Capricoin system."))

    def show_report_bug(self):
        QMessageBox.information(self, "Light-capricoin - " + _("Reporting Bugs"),
            _("Please report any bugs as issues on github:")+" <a href=\"https://github.com/Capricoinofficial/light-capricoin/issues\">https://github.com/Capricoinofficial/light-capricoin/issues</a>")
    ...
    def notify(self, message):
        if self.tray:
            self.tray.showMessage("Light-capricoin", message, QSystemTrayIcon.Information, 20000)
    ...
    def create_send_tab(self):
        ...
        msg = _('Recipient of the funds.') + '\n\n'\
              + _('You may enter a Capricoin address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a Capricoin address)')
        ...
        msg = _('Capricoin transactions are in general not free. A transaction fee is paid by the sender of the funds.') + '\n\n'\
    ...
    def read_send_tab(self):
        ...
        for _type, addr, amount in outputs:
            if addr is None:
                QMessageBox.warning(self, _('Error'), _('Capricoin Address is None'), _('OK'))
                return
            if _type == 'address' and not bitcoin.is_address(addr):
                QMessageBox.warning(self, _('Error'), _('Invalid Capricoin Address'), _('OK'))
                return
            if amount is None:
    ...
    def pay_from_URI(self,URI):
        ...
        try:
            ...
            QMessageBox.warning(self, _('Error'), _('Invalid capricoin URI:') + '\n' + str(e), _('OK'))
            return
    ...
    def new_account_dialog(self, password):
        ...
        e = QLineEdit()
        vbox.addWidget(e)
        msg = _("Note: Newly created accounts are 'pending' until they receive capricoins.") + " " \
    ...
    def tx_from_text(self, txt):
        ...
        if is_hex:
            try:
                ...
                traceback.print_exc(file=sys.stdout)
                QMessageBox.critical(None, _("Unable to parse transaction"), _("Light was unable to parse your transaction"))
                return
    ...
    def read_tx_from_qrcode(self):
        from light_capricoin import qrscanner
        try:
    ...
    def read_tx_from_file(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to read file or no transaction found"), _("Light was unable to open your transaction file") + "\n" + str(reason))
    ...
    def export_privkeys_dialog(self, password):
        ...
        defaultname = 'light-capricoin-private-keys.csv'
        ...
        try:
            ...
            export_error_label = _("Light was unable to produce a private key-export.")
    ...
    def do_import_labels(self):
        ...
        try:
            ...
            QMessageBox.critical(None, _("Unable to import labels"), _("Light was unable to import your labels.")+"\n" + str(reason))


    def do_export_labels(self):
        labels = self.wallet.labels
        try:
            fileName = self.getSaveFileName(_("Select file to save your labels"), 'light-capricoin_labels.dat', "*.dat")
            if fileName:
                with open(fileName, 'w+') as f:
                    json.dump(labels, f)
                QMessageBox.information(None, _("Labels exported"), _("Your labels where exported to")+" '%s'" % str(fileName))
        except (IOError, os.error), reason:
            QMessageBox.critical(None, _("Unable to export labels"), _("Light was unable to export your labels.")+"\n" + str(reason))
    ...
    def export_history_dialog(self):
        ...
        vbox = QVBoxLayout(d)
        defaultname = os.path.expanduser('~/light-capricoin-history.csv')
        ...
        try:
            ...
            export_error_label = _("Light was unable to produce a transaction export.")
    ...
    def settings_dialog(self):
        ...
        d = QDialog(self)
        d.setWindowTitle(_('Light Settings'))
        d.setModal(1)
        vbox = QVBoxLayout()
        grid = QGridLayout()
        ...
        from light_capricoin.i18n import languages
        ...
        if self.need_restart:
            QMessageBox.warning(self, _('Success'), _('Please restart Light to activate the new GUI settings'), _('OK'))



    def run_network_dialog(self):
        if not self.network:
            QMessageBox.warning(self, _('Offline'), _('You are using Light in offline mode.\nRestart Light if you want to get connected.'), _('OK'))
            return
    ...
    def plugins_dialog(self):
        from light_capricoin.plugins import plugins, descriptions, is_available, loader

        self.pluginsdialog = d = QDialog(self)
        d.setWindowTitle(_('Light Plugins'))
        d.setModal(1)
    ...
    def create_csr(self, alias, challenge, password):
        from light_capricoin import x509
        import tlslite

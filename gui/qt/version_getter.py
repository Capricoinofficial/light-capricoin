...
from light_capricoin import ELECTRUM_VERSION, print_error
...
class VersionGetter(threading.Thread):
    ...
    def run(self):
        try:
            con = httplib.HTTPSConnection('www.capricoin.org', timeout=5)
...
class UpdateLabel(QLabel):
    def __init__(self, config, sb):
        ...
        self.connect(self, QtCore.SIGNAL('new_light_version'), self.new_light_version)
    ...
    def callback(self, version):
        ...
        if(self.compare_versions(self.latest_version, self.current_version) == 1):
            ...
            if(self.compare_versions(self.latest_version, latest_seen) == 1):
                ...
                self.emit(QtCore.SIGNAL('new_light_version'))

    def new_light_version(self):
        if self.new_version:
    ...
    def open_website(self):
        webbrowser.open("http://www.capricoin.org/")
        self.dialog.done(0)
    ...
    def mouseReleaseEvent(self, event):
        ...
        main_layout.addWidget(QLabel(_("A new version of Light is available:")+" " + self.latest_version), 0,0,1,3)

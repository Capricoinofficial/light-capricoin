...
from light_capricoin import SimpleConfig, Wallet, WalletStorage, format_satoshis
from light_capricoin.bitcoin import is_address
...
def pay_to(recipient, amount, label):
    ...
    droid.dialogCreateSpinnerProgress("Light-Capricoin", "signing transaction...")
    droid.dialogShow()
        ...
        if event["name"] == "click":
            ...
            if id=="buttonPay":
                ...
                if r:
                    ...
                    if data:
                        print "data", data
                        if re.match('^capricoincoin:', data):
...
def receive_loop():
    ...
    while out is None:
        ...
        if not event:
            ...
            amount = modal_input('Amount', 'Amount you want to receive (in CPC). ', format_satoshis(receive_amount) if receive_amount else None, "numberDecimal")
...
def make_bitmap(data):
    ...
    try:
        import qrcode
        from light_capricoin import bmp
        qr = qrcode.QRCode()
        qr.add_data(data)
...
class LightGui:
    ...
    def __init__(self, config, _network):
        ...
        if not storage.file_exists:
            ...
            droid.dialogCreateSpinnerProgress("Light-Capricoin", msg)
            droid.dialogShow()

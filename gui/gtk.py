...
from light_capricoin.util import print_error, InvalidPassword
from light_capricoin.bitcoin import is_valid
from light_capricoin import WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Light-Capricoin"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from light_capricoin.util import format_satoshis, parse_URI
from light_capricoin.network import DEFAULT_SERVERS
from light_capricoin.bitcoin import MIN_RELAY_TX_FEE
...
def restore_create_dialog():
    ...
    dialog = Gtk.Dialog("light", parent=None, 
                        flags=Gtk.DialogFlags.MODAL,
...
def run_network_dialog( network, parent ):
    ...
    add_help_button(host_box, 'The name, port number and protocol of your Light server, separated by a colon. Example: "ecdsa.org:50002:s". Some servers allow you to connect through http (port 80) or https (port 443)')
    host_box.show()
...
class LightWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Light-Capricoin ' + self.wallet.light_version + '  -  ' + self.config.path
        ...
        from light_capricoin.util import StoreDict
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid capricoin address:\n"+to_address)
            return
...
class LightGui():
    ...
    def main(self, url=None):
        ...
        w = LightWindow(self.wallet, self.config, self.network)

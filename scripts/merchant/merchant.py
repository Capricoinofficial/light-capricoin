...
wallet_path = config.get('light','wallet_path')
xpub = config.get('light','xpub')
...
def check_create_table(conn):
    global num
    c = conn.cursor()
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='light_payments';")
    data = c.fetchall()
    if not data: 
        c.execute("""CREATE TABLE light_payments (address VARCHAR(40), amount FLOAT, confirmations INT(8), received_at TIMESTAMP, expires_at TIMESTAMP, paid INT(1), processed INT(1));""")
        conn.commit()

    c.execute("SELECT Count(address) FROM 'light_payments'")
    num = c.fetchone()[0]
    print "num rows", num
...
def do_dump(password):
    ...
    cur.execute("SELECT oid, * FROM light_payments;")
    data = cur.fetchall()
...
def getrequest(oid, password):
    ...
    cur = conn.cursor()
    # read pending requests from table
    cur.execute("SELECT oid, * FROM light_payments WHERE oid=%d;"%(oid))
    data = cur.fetchone()
...
def db_thread():
    ...
    while not stopping:
        ...
        # read pending requests from table
        cur.execute("SELECT address, amount, confirmations FROM light_payments WHERE paid IS NULL;")
        data = cur.fetchall()
        ...
        if cmd == 'payment':
            ...
            cur.execute("update light_payments set paid=1 where address='%s'"%addr)

        elif cmd == 'request':
            # add a new request to the table.
            addr, amount, confs, minutes = params
            sql = "INSERT INTO light_payments (address, amount, confirmations, received_at, expires_at, paid, processed)"\
                + " VALUES ('%s', %.8f, %d, datetime('now'), datetime('now', '+%d Minutes'), NULL, NULL);"%(addr, amount, confs, minutes)
            print sql
            cur.execute(sql)

        # set paid=0 for expired requests 
        cur.execute("""UPDATE light_payments set paid=0 WHERE expires_at < CURRENT_TIMESTAMP and paid is NULL;""")

        # do callback for addresses that received payment or expired
        cur.execute("""SELECT oid, address, paid from light_payments WHERE paid is not NULL and processed is NULL;""")
        data = cur.fetchall()
        for item in data:
            ...
            try:
                ...
                cur.execute("UPDATE light_payments SET processed=1 WHERE oid=%d;"%(oid))
...
if __name__ == '__main__':
    ...
    # start network
    c = light.SimpleConfig({'wallet_path':wallet_path})
    daemon_socket = light.daemon.get_daemon(c, True)
    network = light.NetworkProxy(daemon_socket, config)
    network.start()
    ...
    storage = light.WalletStorage(c)
    if not storage.file_exists:
        print "creating wallet file"
        wallet = light.wallet.Wallet.from_xpub(xpub, storage)
    else:
        wallet = light.wallet.Wallet(storage)

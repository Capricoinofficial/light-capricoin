...
class Test_SimpleConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.light_dir = tempfile.mkdtemp()
        # Do the same for the user dir to avoid overwriting the real configuration
        # for development machines with light installed :)
        self.user_dir = tempfile.mkdtemp()

        self.options = {"light_path": self.light_dir}
    ...
    def test_simple_config_command_line_overrides_everything(self):
        ...
        sources"""
        fake_read_system = lambda : {"light_path": "a"}
    ...
    def test_simple_config_system_config_ignored_if_portable(self):
        """If light is started with the "portable" flag, system
        configuration is completely ignored."""
        another_path = tempfile.mkdtemp()
        fake_read_system = lambda : {"light_path": self.light_dir}
        fake_read_user = lambda _: {"light_path": another_path}
        read_user_dir = lambda : self.user_dir
        config = SimpleConfig(options={"portable": True},
                              read_system_config_function=fake_read_system,
                              read_user_config_function=fake_read_user,
                              read_user_dir_function=read_user_dir)
        self.assertEqual(another_path, config.get("light_path"))
    ...
    def test_simple_config_user_config_is_used_if_others_arent_specified(self):
        ...
        fake_read_user = lambda _: {"light_path": self.light_dir}
        read_user_dir = lambda : self.user_dir
        config = SimpleConfig(options=None,
                              read_system_config_function=fake_read_system,
                              read_user_config_function=fake_read_user,
    ...
    def test_can_set_options_set_in_user_config(self):
        ...
        config = SimpleConfig(options={},
                              ...
                              read_user_config_function=fake_read_user,
                              read_user_dir_function=read_user_dir)
        config.set_key("light_path", another_path)
        self.assertEqual(another_path, config.get("light_path"))
    ...
    def test_user_config_is_not_written_with_read_only_config(self):
        ...
        contents = None
        with open(os.path.join(self.light_dir, "config"), "r") as f:
            contents = f.read()
...
"""
    ...
    def setUp(self):
        ...
        self.thefile = tempfile.mkstemp(suffix=".light-capricoin.test.conf")[1]

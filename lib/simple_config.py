...
SYSTEM_CONFIG_PATH = "/etc/light-capricoin.conf"
...
class SimpleConfig(object):
    ...
    def init_path(self):
        # Read light path in the command line configuration
        self.path = self.read_only_options.get('light_path')
        ...
        print_error( "light directory", self.path)

    def set_key(self, key, value, save = True):
        if not self.is_modifiable(key):
            print "Warning: not changing key '%s' because it is not modifiable" \
                  " (passed as command line option or defined in /etc/light-capricoin.conf)"%key
            return
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "light-capricoin.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/light-capricoin.conf."""
    result = {}
    if os.path.exists(path):
        try:
            import ConfigParser
        except ImportError:
            print "cannot parse light-capricoin.conf. please install ConfigParser"
            return
...
def read_user_config(path):
    """Parse and store the user config settings in light-capricoin.conf into user_config[]."""
    if not path:
        return {}

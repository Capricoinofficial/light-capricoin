...
from light_capricoin.account import BIP32_Account
from light_capricoin.bitcoin import EncodeBase58Check, public_key_to_bc_address, bc_address_to_hash_160
from light_capricoin.i18n import _
from light_capricoin.plugins import BasePlugin, hook, always_hook, run_hook
from light_capricoin.transaction import deserialize
from light_capricoin.wallet import BIP32_HD_Wallet
from light_capricoin.util import print_error
from light_capricoin.wallet import pw_decode, bip32_private_derivation, bip32_root
...
class TrezorWallet(BIP32_HD_Wallet):
    ...
    def show_address(self, address):
        ...
        try:
            ...
            give_error(e)
        try:
            self.get_client().get_address('Capricoin', address_n, True)
        except Exception, e:
            give_error(e)
        finally:
    ...
    def sign_message(self, address, message, password):
        ...
        try:
            ...
            give_error(e)
        try:
            msg_sig = self.get_client().sign_message('Capricoin', address_n, message)
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
    ...
    def sign_transaction(self, tx, password):
        ...
        try:
            signed_tx = self.get_client().sign_tx('Capricoin', inputs, outputs)[1]
        except Exception, e:
            give_error(e)
        finally:
            self.plugin.handler.stop()
    ...
    def light_tx_to_txtype(self, tx):
    ...
    def get_tx(self, tx_hash):
        ...
        tx.deserialize()
        return self.light_tx_to_txtype(tx)
...
class TrezorQtHandler:
    ...
    def passphrase_dialog(self):
        from light_capricoin_gui.qt.password_dialog import make_password_dialog, run_password_dialog
        d = QDialog()
        d.setModal(1)
